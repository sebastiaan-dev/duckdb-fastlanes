...
def gen_dec_alp_opr_dbl(n_rows, n_cols):
    ...
    for i in range(n_rows):
        for j in range(n_cols):
            data[i][j] = round(Decimal(str(random.uniform(0, 100))) / 100, 4)
...
name_to_func = {
    "dec_uncompressed_opr_uint8": gen_dec_uncompressed_opr_uint8,
    "dec_uncompressed_opr_int8": gen_dec_uncompressed_opr_int8,
    "dec_uncompressed_opr_int16": gen_dec_uncompressed_opr_int16,
    "dec_uncompressed_opr_int32": gen_dec_uncompressed_opr_int32,
    "dec_uncompressed_opr_int64": gen_dec_uncompressed_opr_int64,
    "dec_uncompressed_opr_dbl": gen_dec_uncompressed_opr_dbl,
    "dec_uncompressed_opr_str": gen_dec_uncompressed_opr_str,
    ########################################
    "dec_unffor_opr_uint8": gen_dec_unffor_opr_uint8,
    "dec_unffor_opr_int8": gen_dec_unffor_opr_int8,
    "dec_unffor_opr_int16": gen_dec_unffor_opr_int16,
    "dec_unffor_opr_int32": gen_dec_unffor_opr_int32,
    "dec_unffor_opr_int64": gen_dec_unffor_opr_int64,
    ########################################
    "dec_alp_opr_dbl": gen_dec_alp_opr_dbl,
    "dec_alp_rd_opr_dbl": gen_dec_alp_rd_opr_dbl,
    ########################################
    "dec_constant_opr_uint8": gen_dec_constant_opr_uint8,
    "dec_constant_opr_int8": gen_dec_constant_opr_int8,
    "dec_constant_opr_int16": gen_dec_constant_opr_int16,
    "dec_constant_opr_int32": gen_dec_constant_opr_int32,
    "dec_constant_opr_int64": gen_dec_constant_opr_int64,
    "dec_constant_opr_dbl": gen_dec_constant_opr_dbl,
    "dec_constant_opr_str": gen_dec_constant_opr_str,
    ########################################
    "dec_fsst_opr_str": gen_dec_fsst_opr_str,
    "dec_fsst12_opr_str": gen_dec_fsst12_opr_str,
    ########################################
    "dec_null_opr_int16": gen_dec_null_opr_int16,
    "dec_null_opr_int32": gen_dec_null_opr_int32,
    "dec_null_opr_dbl": gen_dec_null_opr_dbl,
    ########################################
    "dec_slpatch_opr_i16": None,  # Placeholder for the function
    ########################################
    "dec_frequency_opr_int8": gen_dec_frequency_opr_int8,
    "dec_frequency_opr_int16": gen_dec_frequency_opr_int16,
    "dec_frequency_opr_int32": gen_dec_frequency_opr_int32,
    "dec_frequency_opr_int64": gen_dec_frequency_opr_int64,
    "dec_frequency_opr_dbl": gen_dec_frequency_opr_dbl,
    "dec_frequency_opr_str": gen_dec_frequency_opr_str,
    ########################################
    "dec_cross_rle_opr_int8": gen_dec_cross_rle_opr_int8,
    "dec_cross_rle_opr_int16": gen_dec_cross_rle_opr_int16,
    "dec_cross_rle_opr_int32": gen_dec_cross_rle_opr_int32,
    "dec_cross_rle_opr_int64": gen_dec_cross_rle_opr_int64,
    "dec_cross_rle_opr_dbl": gen_dec_cross_rle_opr_dbl,
    "dec_cross_rle_opr_str": gen_dec_cross_rle_opr_str,
}


def generate_all_encodings(cols):
    """
    ...
    n_rowgroups = 20
    n_vectors_per_rowgroup = 64
    n_vectors = n_rowgroups * n_vectors_per_rowgroup

    for n_cols in [cols]:
        # for n_vectors in range(
        #     n_vectors_per_rowgroup, n_vectors + 1, n_vectors_per_rowgroup
        # ):
        for n_vectors in [n_vectors]:
            n_rows = n_vectors * 1024

            for name, func in name_to_func.items():
                if func is None:
                    continue
                if "dec_alp_opr_dbl" not in name:
                    continue
                data, col_type = func(n_rows, n_cols)
                result[f"{name}_{n_cols}_{n_vectors}"] = (data, col_type)
...
def write_all_encodings_to_csv():
    ...
    for n_occurence in range(1, repeat + 1):
        encodings = generate_all_encodings(cols)

        for name, (data, col_type) in encodings.items():
            path = f"source/{name}_{n_occurence}"
            os.makedirs(path, exist_ok=True)
            write_to_csv(data, f"{path}/data.csv")
            write_to_json_schema(
                [col_type.value for _ in range(cols)], f"{path}/schema.json"
            )
