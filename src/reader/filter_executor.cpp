...
void FilterExecutor::Apply(DataChunk&                              chunk,
	...
	SelectionVector selection(scan_count);
	for (idx_t i = 0; i < scan_count; ++i) {
		selection.set_index(i, static_cast<sel_t>(i));
	}
	...
	for (idx_t filter_idx = 0; filter_idx < scan_filters.size(); filter_idx++) {
		...
		ColumnSegment::FilterSelection(
		    selection, result_vector, unified, scan_filter.filter, *scan_filter.filter_state, scan_count, filter_count);
	...
	if (active_count != scan_count) {
		chunk.Slice(selection, active_count);
	}
}
